...
public class EventCallbackTest {
    ...
    @Test
    public void testEventQueueFull(){
        EventExecuteService ees = mock(EventExecuteService.class);
        when(ees.add(new EventDummy())).thenThrow();

        EventCallback eventCallback = new EventCallback(ees, LOGGER_EVENT);
        eventCallback.onEvent(new EventDummy());
    }
}
